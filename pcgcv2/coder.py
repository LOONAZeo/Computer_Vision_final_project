...
class CoordinateCoder():
    """encode/decode coordinates using bzip2
    """
    ...
    def encode(self, coords, postfix=''):
        coords_bytes = coords.numpy().astype(np.int32).tobytes()
        compressed_bytes = bz2.compress(coords_bytes)

        return compressed_bytes

    def decode(self, compressed_bytes,postfix=''):
        decompressed_data = np.frombuffer(bz2.decompress(compressed_bytes), dtype=np.int32).reshape(-1, 2)
        decoded_coords = torch.tensor(decompressed_data)
...
class Coder():
    ...
    def encode(self, x, postfix=''):
        ...
        # [2, 3] means it only selects [H, W] as the coordinates to be compressed by bzip2
        # it uses gpcc to compress coordinates when it comes to point cloud
        encoded_coords = self.coordinate_coder.encode((y.C//y.tensor_stride[0]).detach().cpu()[:, [2, 3]], postfix=postfix)

        return y, encoded_coords

    @torch.no_grad()
    # def decode(self, rho=1, postfix=''):
    def decode(self, encoded_coords, postfix=''):
        # decode coords
        y_C = self.coordinate_coder.decode(encoded_coords, postfix=postfix)
        y_C = torch.cat((torch.zeros((len(y_C),1)).int(), torch.zeros((len(y_C),1)).int(), torch.tensor(y_C).int()), dim=-1)
        ...
        # decode
        out = self.model.decoder(y, nums_list=num_points, ground_truth_list=[None]*3, training=False)
