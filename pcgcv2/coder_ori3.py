...
from data_utils import write_ply_ascii_geo, read_ply_ascii_geo, write_ply_ascii_geo_for_2D
...
class CoordinateCoder():
    ...
    def encode(self, coords, postfix=''):
        ...
        write_ply_ascii_geo_for_2D(filedir=self.ply_filename, coords=coords)
...
class Coder():
    ...
    def encode(self, x, postfix=''):
        ...
        z1_list = self.model.encoder1(x)
        z1 = z1_list[0]
        xp = self.model.decoder1(z1)
        x1 = x - xp
        xpp_list = self.model.encoder2(x1)
        xpp = xpp_list[0]
        z2 = xpp + z1

        num_points = [len(ground_truth) for ground_truth in xpp_list[1:] + [x]]
        with open(self.filename+postfix+'_num_points.bin', 'wb') as f:
            f.write(np.array(num_points, dtype=np.int32).tobytes())
        self.feature_coder.encode(z2.F, postfix=postfix)
        self.coordinate_coder.encode((z2.C//z2.tensor_stride[0]).detach().cpu()[:,1:], postfix=postfix)
        
        return z2

    @torch.no_grad()
    def decode(self, rho=1, postfix='', y_C=''):
        y_F = self.feature_coder.decode(postfix=postfix)

        y = ME.SparseTensor(
            features=y_F,
            coordinate_map_key=y_C.coordinate_map_key,
            coordinate_manager=y_C.coordinate_manager,
            device=device)

        # out = self.model.decoder(y)

        x1_q = self.model.decoder2(y)
        xs_list = self.model.encoder2(x1_q)
        xs = xs_list[0]
        z1_q = y - xs
        out = x1_q + self.model.decoder1(z1_q)
        ...
        return out
        # out = self.model.decoder(z, nums_list=None, ground_truth_list=[None]*3, training=False)
        #
        # return out
