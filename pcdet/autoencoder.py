...
def dense_to_sparse(tensor):
    ...
    # values = tensor[nonzero_indices[:, 0], nonzero_indices[:, 1], nonzero_indices[:, 2], nonzero_indices[:, 3]]
    values = tensor[nonzero_indices[:, 0], nonzero_indices[:, 1], nonzero_indices[:, 2]]
    features = values.view(-1, 1)
    nonzero_indices = nonzero_indices.int().contiguous()
    # sparse_tensor = ME.SparseTensor(coordinates=nonzero_indices, features=features.float(), device=device)
    return features.float(), nonzero_indices

def sparse_to_dense(sparse_tensor, output_shape):
    # Output shape would be (B, C, H, W) in your case
    # Create a dense tensor filled with zeros
    dense_tensor = torch.zeros(output_shape, dtype=sparse_tensor.dtype).cuda()
    dense_tensor = dense_tensor.unsqueeze(1)

    # Get coordinates and features from the sparse tensor
    coordinates = sparse_tensor.coordinates
    features = sparse_tensor.features

    # Assuming coordinates are batched, and you are working with a 4D dense tensor
    for i in range(coordinates.shape[0]):
        # B, C, H, W = coordinates[i].tolist()  # Adjust based on actual coordinate format
        B, H, W = coordinates[i].tolist()  # Adjust based on actual coordinate format
        # dense_tensor[B, :, H, W] = features[i]  # Assuming single-channel features
        dense_tensor[B, :, H, W] = features[i]  # Assuming single-channel features

    return dense_tensor.squeeze(1)