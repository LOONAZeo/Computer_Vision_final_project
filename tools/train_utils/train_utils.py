...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
    ...
    for cur_it in range(start_it, total_it_each_epoch):
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            # loss, tb_dict, disp_dict, sum_loss, bpp, mae, mse_loss = model_func(model, batch)
            loss, tb_dict, disp_dict, sum_loss, bpp, bce = model_func(model, batch)
        ...
        if rank == 0:
            ...
            disp_dict.update({
                'loss': loss.item(), 'lr': cur_lr, 'd_time': f'{data_time.val:.2f}({data_time.avg:.2f})', 'sum_loss': sum_loss.item(),
                'bpp': bpp.item(), 'bce': bce.item(),
            ...
            if use_logger_to_record:
                if accumulated_iter % logger_iter_interval == 0 or cur_it == start_it or cur_it + 1 == total_it_each_epoch:
                    ...
                    # Log sum_loss separately
                    logger.info(f'Sum Loss: {disp_dict.get("sum_loss", "not found")}')
                    logger.info(f'Bpp: {disp_dict.get("bpp", "not found")}')
                    logger.info(f'Bce: {disp_dict.get("bce", "not found")}')
